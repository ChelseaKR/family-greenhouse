...
**Run `npm ci` after you pull, too.** Before it schedules anything, the gate compares `node_modules` against `package-lock.json` and stops if they disagree (`scripts/check-dependency-freshness.mjs`, also runnable on its own as `npm run deps:check`). It costs ~0.2s and it distinguishes the two things that used to look identical: a package the lockfile requires and your tree does not have, versus one installed at a version the lockfile does not pin. Both say `npm ci`; neither is a problem with your change. Without it, a stale install surfaces as whichever gate step happened to import the missing package first — on 2026-09-05 that was `gate FAILED — 1 of 28 steps failed in 83.3s: test:backend`, for fifteen suites that could not import a devDependency added 139 commits earlier (#581).

The same script also sets `gpg.ssh.allowedSignersFile` to the committed [`.github/allowed_signers`](.github/allowed_signers), so `git tag -v v0.28.0` verifies a release tag locally the way `cd-production.yml` does in CI (#582). Without it git reports `gpg.ssh.allowedSignersFile needs to be configured and exist for ssh signature verification` on a perfectly good signature — a false negative, in the position where a bad-signature error would appear. It exits 1 in that state, so it fails closed; set it by hand with `git config gpg.ssh.allowedSignersFile .github/allowed_signers` if you skipped install. An existing value is left alone rather than overwritten.
...
Run locally before pushing: `make verify` (the portfolio-standard entry point, which runs `npm run verify`) — the same stages CI runs: `format:check`, `lint`, `typecheck`, `test:coverage`, `i18n:check`, `reads:check`, `observability:check`, `npm audit (--omit=dev --audit-level=high)`, `secrets:check` (gitleaks over both the commit history and the files on disk — it fails rather than skips if gitleaks isn't installed, so run `scripts/setup-dev.sh`), `lint:scripts` (ESLint over the `.mjs` gate scripts and the infrastructure Lambdas), `shell:check` (shellcheck over every tracked `.sh`), the bare-marker / silenced-gate / docs-testing guards, and the figures, API-spec, sitemap and brand checks. `test:coverage` is `vitest run --coverage` in both workspaces, so the coverage floors that gate `Test Frontend` / `Test Backend` are enforced locally too, not only in CI. `.githooks/pre-push` already calls it.
...
- The pool sizes itself for the machine it is **actually** on. `availableParallelism()` reports cores, not free cores, so concurrent gate runs used to size N pools as though each were alone — three of them turned a 58-81s frontend suite into 987s and failed it on timeouts, on branches touching no frontend file (#596). [`scripts/gate-census.mjs`](scripts/gate-census.mjs) counts the gate runs in the process table and each takes `cores / that`, re-asked while it runs, and passed down as `GATE_PEERS` so the vitest pools divide too. `node scripts/gate-census.mjs` prints the current count and the width it implies.
