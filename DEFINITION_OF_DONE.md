...
| `Security Scan`                                    | `ci.yml`                       | `npm audit` on production deps (HIGH+CRITICAL block); secret-scan coverage gate (`scripts/check-secret-scan-coverage.mjs`: default ruleset loaded, no tracked file allowlisted out); Gitleaks over git history **and** the working tree, each gating on its own exit code (blocking, no `\|\| true`) |
