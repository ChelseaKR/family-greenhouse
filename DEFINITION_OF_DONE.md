...
| -------------------------------------------------- | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Lint`                                             | `.github/workflows/ci.yml`     | ESLint zero errors; Prettier `--check`; no bare TODO/FIXME/HACK markers; no silenced test/security/lint gates in workflows; API spec covers every handler route; `docs/testing.md` matches the suite; observability/SLO contract; settled-read-state ratchet (ADR 0010); commitlint                                                                   |
...
| `Build`                                            | `ci.yml`                       | frontend + backend build artifacts; SEO gate over `frontend/dist` (`scripts/check-seo-build.mjs`): per-route unique title + description within length bounds, exactly one `<h1>`, absolute self-referencing apex canonical matching `og:url`, sitemap ↔ prerendered-route parity, no page for a `Disallow`ed route, no loopback address in built HTML |
...
bare-marker, silenced-gates and docs-testing guards + `seo:check` — the local
mirror of the CI gates that have no browser/cloud dependency. `seo:check`
builds the frontend first, because the thing it checks is the built output;
it is the same script CI's `Build` job runs against the artifact it uploads. It runs `vitest run --coverage` in both
...
Last verified: 2026-08-28 (against `.github/workflows/` and ruleset id 18752847) ·
