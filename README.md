...
| -------------------------- | ------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Responsible-Tech Framework | Applies (PII + AI)                                                              | Partial — §A–F audit, dated DPIA, AI risk register, model card, EU classification, disclosure, and live grounding/privacy controls are committed; ISO 42001 SoA and a published ACR/VPAT remain gaps                                                                                           |
...
| Release & Versioning       | Applies — tagged releases drive prod deploys                                    | Partial — root/workspace versions are aligned at 0.27.0; tag/version and CHANGELOG gates run at the tagged ref. Release SBOM/provenance and blocking signed-tag verification remain open                                                                                                       |
...
| Internationalization       | Applies (opted in — EN/ES)                                                      | Partial — `docs/i18n.md`, EN/ES key/placeholder/plural/TODO gates, UTF-8 validation, and a hardcoded-string ratchet exist; native-speaker sign-off, pseudolocale overflow, RTL logical-property lint, and remaining literal migration remain                                                   |
| AI Evaluation              | Applies — production Bedrock chat (tool-use + RAG) + leaf-health vision         | Dated waiver (expires 2026-10-05) — starter benchmark + live quantitative grounding block + model card; full RAGAS-class metric suite and red-team scan are not yet built. Judge calibration remains N/A — no LLM-as-judge is used; see `docs/RESPONSIBLE-TECH-AUDITS.md`                      |
| Documentation              | Applies                                                                         | Met for current scope — README declaration, DoD, CHANGELOG, citation metadata, ADRs, API contracts, operational runbooks, and dated audit artifacts are committed and drift-gated where mechanical                                                                                             |
| Quality & Metrics          | Applies                                                                         | Partial — exact merge gates and the DoD are committed in `DEFINITION_OF_DONE.md`; automated portfolio DORA reporting and the seven-part AI-capabilities governance record remain gaps                                                                                                          |
| AI Development Measurement | Applies — repository ledger gap                                                 | Development uses AI-assisted tooling; the portfolio Track A metrics apply, while the repository-specific roadmap declaration and baseline remain to be added                                                                                                                                   |
| Incident Response          | Applies                                                                         | [`docs/incidents.md`](docs/incidents.md) defines severity, first-response, communications, and postmortem practice for the deployed service                                                                                                                                                    |
| Data Governance            | Applies — L3 PII                                                                | The [DPIA](docs/audits/dpia.md) inventories PII, storage, deletion, and retention; remaining lifecycle gaps stay explicit there                                                                                                                                                                |

**On the CI/CD row:** the committed `protect-main` ruleset carries exactly one bypass actor — the repository **admin role**, `bypass_mode: always` — as the owner's break-glass path. An empty bypass list is not a stricter gate; it removes the only way to repair a wedged `main`, and GitHub returns 201 when you apply one. That single entry is a deliberate, recorded divergence from the vendored `CI-CD-STANDARD.md` CICD-15 (PR-only bypass) and §5.1 (empty bypass list) — reasoning, the live-versus-committed state, and the owner-only reconcile command are in [`.github/rulesets/README.md`](.github/rulesets/README.md).
...
**Finding IDs.** Some code comments and ADRs cite short IDs — `CQ-16`, `CICD-27`, `DOC-11`, `P1-5`, `P2-7`. They come from the dated 2026-07-05 conformance audit of this repo and its remediation plan, cited by [ADR 0006](docs/adr/0006-standards-applicability-declarations.md). The private audit workspace is not part of this repository, so the IDs are traceable labels, not links. `CQ-*` / `CICD-*` / `DOC-*` / `RTF-*` number per-standard findings; `P1-*` / `P2-*` / `P3-*` number the prioritized remediation items. The one cited most often in code: **CQ-16 / P1-5 — coverage floors sit below the CODE-QUALITY standard's 80% target and must ratchet upward release over release instead of staying pinned.** That is the policy the `thresholds` comments in `frontend/vitest.config.ts` and `backend/vitest.config.ts` are enforcing.
