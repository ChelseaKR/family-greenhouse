...
| CI/CD                      | Applies                                                                         | Met for current service shape — committed ruleset (one bypass actor: the owner's standing admin bypass, see `.github/rulesets/README.md`) + CODEOWNERS, strict required checks, OIDC deploys, workflow audit, concurrency, and `make verify` parity; production deployment retains its explicit Environment approval |
