...
| [SECURITY-AND-SUPPLY-CHAIN](docs/standards/SECURITY-AND-SUPPLY-CHAIN-STANDARD.md) | Applies — **ASVS L2** (real PII: emails, phones, photos, household graphs)      | Gap tracked — no SBOM/signing; CodeQL and OpenSSF Scorecard now run (see `.github/workflows/codeql.yml`, `scorecard.yml`); gitleaks pre-commit + version bump added this pass                                                               |
| [CI-CD](docs/standards/CI-CD-STANDARD.md)                                         | Applies                                                                         | Gap tracked — no branch-ruleset artifact, no `make verify` parity (added this pass); zizmor now runs (`.github/workflows/zizmor.yml`); skip-lighthouse bypass closed and staging e2e silencing removed this pass                            |
