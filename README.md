...
| ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Quality & Metrics](docs/standards/QUALITY-AND-METRICS-STANDARD.md)             | Applies                                                                         | Partial — exact merge gates and the DoD are committed in `DEFINITION_OF_DONE.md`; automated portfolio DORA reporting and the seven-part AI-capabilities governance record remain gaps                                                                                     |
| [Code Quality](docs/standards/CODE-QUALITY-STANDARD.md)                         | Applies (TS/Node; Python N/A — no Python code in this repo)                     | Partial — strict TypeScript, zero-warning ESLint, coverage floors, CODEOWNERS, and branch rules are gated. Backend clears 80% on all four coverage metrics; frontend floors are ratcheted below the portfolio's 80% target                                                |
| [Security & Supply-Chain](docs/standards/SECURITY-AND-SUPPLY-CHAIN-STANDARD.md) | Applies — **ASVS L2** (real PII: emails, phones, photos, household graphs)      | Partial — CodeQL, Semgrep, Gitleaks, npm audit, Scorecard, SHA-pinned Actions, and a complete dependency disposition are gated/evidenced; release SBOM, provenance, and signed-tag enforcement remain gaps                                                                |
...
| [Release & Versioning](docs/standards/RELEASE-AND-VERSIONING-STANDARD.md)       | Applies — tagged releases drive prod deploys                                    | Partial — root/workspace versions are aligned at 0.23.0; tag/version and CHANGELOG gates run at the tagged ref. Release SBOM/provenance and blocking signed-tag verification remain open                                                                                  |
...
**Finding IDs.** Some code comments and ADRs cite short IDs — `CQ-16`, `CICD-27`, `DOC-11`, `P1-5`, `P2-7`. They come from the dated 2026-07-05 conformance audit of this repo and its remediation plan (`portfolio/audit-2026-07-05/family-greenhouse-AUDIT.md`, cited by [ADR 0006](docs/adr/0006-standards-applicability-declarations.md)); that audit workspace is not part of this repository, so the IDs are traceable labels, not links. `CQ-*` / `CICD-*` / `DOC-*` / `RTF-*` number per-standard findings; `P1-*` / `P2-*` / `P3-*` number the prioritized remediation items. The one cited most often in code: **CQ-16 / P1-5 — coverage floors sit below the CODE-QUALITY standard's 80% target and must ratchet upward release over release instead of staying pinned.** That is the policy the `thresholds` comments in `frontend/vitest.config.ts` and `backend/vitest.config.ts` are enforcing.
