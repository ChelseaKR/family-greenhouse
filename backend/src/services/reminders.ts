...
import { reminderDeliveryTime, isBeforeReminderDeliveryTime } from './deliveryTime.js';
...
// The HOUR a daily reminder goes out — `reminderDeliveryTime` and
// `isBeforeReminderDeliveryTime` — lives in `services/deliveryTime.ts` so the
// household chat channel (#674) keeps the same rule without importing this
// module's whole fan-out. Re-exported so existing callers are unchanged.
export {
  REMINDER_DEFAULT_DELIVERY_TIME,
  reminderDeliveryTime,
  isBeforeReminderDeliveryTime,
} from './deliveryTime.js';
