...
- **Gate:** AUTO-GATE — the citation/grounding guard is unit-tested and wired into the live `turnEvents` RAG path. It checks recognized care-relevant quantitative claims (frequency, percentage, temperature, duration, length, volume, mass, dose/dilution, repetition, and fertilizer ratios) against retrieved spans before persistence or delivery; a failed answer is replaced by a safe verification message. RAG streaming text is held until the same guard passes, with sync and streaming regression tests. **The guard reports a three-state verdict and never a bare pass:** an answer in which it recognized no checkable claim is reported (and logged) as `unverified`, not grounded, so "we traced this answer's quantities to the corpus" and "we recognized nothing in this answer" are different facts here and in the logs. `unverified` answers are still delivered — the guard is a quantitative heuristic, not a semantic-entailment check, and most care answers are qualitative — so this gate must not be read as "every delivered answer was verified"; it means no answer delivered with a recognized quantitative claim contradicts the corpus. **One class of qualitative claim is gated since 2026-09-02:** a categorical pet-safety claim ("safe for cats", "non-toxic", and the Spanish forms) is a recognized claim, grounded only by a `non-toxic` verdict from the `check_pet_toxicity` tool for the plant and species it names; an unsupported one blocks and is replaced by a refusal that points at the verified checker and the ASPCA poison-control line — see [ADR 0011](adr/0011-categorical-pet-safety-claims-block.md). **A third class is gated since 2026-09-05:** a count or totality claim about the user's own collection ("you have 100 plants", "none of your plants are toxic") is checked against how much of the household the answering service was actually sent, since the Sprout payload is a subset of it twice over; an unsupported one blocks unless the sentence states the household total — see [ADR 0026](adr/0026-household-counts-over-a-partial-payload-block.md). The deterministic starter benchmark still checks retrieval recall only; the full live-model scoring waiver below remains in force. See [ADR 0008](adr/0008-unit-aware-rag-grounding.md) and [ADR 0009](adr/0009-three-state-grounding-verdict.md).
...
| `docs/audits/red-team-<date>.md`           | ⚠️ partial — `docs/audits/red-team-2026-07-17.md` committed: an offline, CI-gated prompt-injection corpus over the chat **tool layer** (household-scope, PII redaction, write-gate; OWASP LLM01/02/06 data-layer). Live-model refusal/no-fabrication, Promptfoo, and Garak remain not-run (§2 waiver still in force). |
