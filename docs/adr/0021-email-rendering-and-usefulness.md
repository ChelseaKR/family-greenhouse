...
| Remote image loads (tracking pixels, spoofed logos, IP disclosure) | No `<img>` at all. (Until ADR 0033 the one exception was a plant photo on our own asset origin, checked by `isOwnAssetUrl`; photos are now served only through short-lived signed URLs, which cannot live in an inbox.) There are no tracking pixels, no external CSS, and no web fonts.                                                             |
