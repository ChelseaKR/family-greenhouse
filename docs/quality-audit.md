...
> routes stale (the handlers now expose 115) while truthfully describing a passing gate.
...
3. **Localization content gap.** Spanish is reachable since #467 (browser-language detection and the Settings picker), and its first screens (landing page, pricing hero, public footer, 404) are translated and render-tested, but copy that never went through the catalog — the long-form help/care/blog/changelog content, `/pet-safe`, the password-reset pages, and the JSX the hardcoded-string ratchet still baselines — renders in English on a Spanish screen, and the catalog has had no native-speaker review. The catalog itself is guarded: `frontend/src/i18n/coverage.ts` + `tests/unit/i18n/localeCoverage.test.ts` assert every non-English locale defines all keys (no silent fallbacks) and clears 95% translated on every run. See `docs/i18n.md` § Shipping status.
