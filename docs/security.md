...
- **Reply-to-act inbound mail** (`backend/src/services/emailReplies.ts`, [ADR 0031](adr/0031-reply-to-act-on-reminder-emails.md), off by default): a reply to a reminder can complete or snooze only the tasks that one email listed, and only when the per-email token (stored as its scrypt digest), the member's stored address as the single From mailbox, and a DMARC `PASS` all agree. The body is read for one line against a closed grammar (`services/email/replyCommand.ts`); answers go to the member's stored address with our own subject and body, and the only inbound value echoed into a header is a Message-ID proved to be printable ASCII in angle brackets.

**Verified by**: a per-handler unit test mocks the DDB client and asserts on the parameters passed to each command. For the forwarder, `sesForwarder.test.ts` asserts that a `From` carrying a bare CR or LF yields no extra header line. For reply-to-act, `tests/unit/services/emailReplies.test.ts` runs the real outbound sender path and asserts that neither a bare-LF `From` nor a CRLF-carrying Message-ID produces a header line, alongside the foreign-token, wrong-sender, expired and replay cases.
...
- **MFA**: optional software-token TOTP, enrolled by the user under Settings → Security (#671). Cognito generates and stores the secret (`AssociateSoftwareToken`), checks the first code before the factor is switched on (`VerifySoftwareToken`, then `SetUserMFAPreference`), and checks every sign-in code (`RespondToAuthChallenge`). The app stores no secret and logs neither a secret nor a code; the secret crosses the API once, in the setup response, with `Cache-Control: no-store`, and the QR code is drawn in the browser. Setup asks for the current password; turning the factor off asks for the password AND a current code. Those re-authentication steps protect the app's own surface — someone at an unlocked, signed-in device cannot bind their own authenticator or remove the owner's through Settings — but they are **not** a hard boundary against a stolen access token: Cognito's public self-service API accepts that token for the same calls directly. That residual is inherent to a public app client and bounded by the one-hour access-token lifetime. There are no recovery codes yet (the account is recovered by support; see docs/runbooks.md, "Reset a user's two-step verification"). SMS MFA is deliberately off.
...
```
default-src 'self'; script-src 'self' https://www.googletagmanager.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: __API_ORIGIN__ https: https://*.google-analytics.com https://*.analytics.google.com; connect-src 'self' __API_ORIGIN__ https: https://*.google-analytics.com https://*.analytics.google.com; font-src 'self' data:; object-src 'none'; base-uri 'self'; form-action 'self' https:; manifest-src 'self'
```

`__API_ORIGIN__` is substituted with the deployed API origin at build time. Three things this policy does that a reader should not have to infer:

- **`script-src` admits exactly one third-party origin: `https://www.googletagmanager.com`, for Google Analytics 4's `gtag.js` on the website** (the 2026-09-17 decision; `docs/analytics.md`, "Google Analytics 4"). `frontend/src/services/googleAnalytics.ts` injects it only when the production web build carried a measurement ID and no opt-out applies, and never inside the native shells. Its hits go to `*.google-analytics.com` and `*.analytics.google.com`, which both policies name in `connect-src` and `img-src`. Nothing else from Google is admitted — no Tag Manager container, and gtag.js's mirror of each hit to `www.google.com/g/collect` is blocked by the CloudFront policy on purpose. (From 2026-09-13 to 2026-09-17 `script-src` was `'self'` alone, after a never-keyed GTM loader was removed.)
- **Product analytics leave the browser over `connect-src`, not `script-src`.** `frontend/src/services/analytics.ts` is a `fetch` shim that POSTs to PostHog's capture endpoint; the meta policy's broad `https:` allowance (which exists for Stripe Checkout and Plant.id) covers it, and the CloudFront response-headers policy in `infrastructure/modules/frontend/main.tf` — the stricter, authoritative one — names `us.i.posthog.com` and `eu.i.posthog.com` explicitly, alongside `*.sentry.io` for the shipped-but-unkeyed error rail.
