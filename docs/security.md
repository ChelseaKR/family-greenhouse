...
```
default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: __API_ORIGIN__ https:; connect-src 'self' __API_ORIGIN__ https:; font-src 'self' data:; object-src 'none'; base-uri 'self'; form-action 'self' https:; manifest-src 'self'
```
...
- **`script-src` is `'self'` alone: no third-party script is ever loaded.** Until 2026-09-13 it admitted `https://www.googletagmanager.com` for a Google Tag Manager loader that no environment had ever keyed; that loader was removed with the analytics decision in `docs/analytics.md` (PostHog only, cookieless — GTM/GA4 set cookies and would have voided that posture), and the allowance went with it, so the policy no longer permits what the code no longer does.
- **Product analytics leave the browser over `connect-src`, not `script-src`.** `frontend/src/services/analytics.ts` is a `fetch` shim that POSTs to PostHog's capture endpoint; the meta policy's broad `https:` allowance (which exists for Stripe Checkout and Plant.id) covers it, and the CloudFront response-headers policy in `infrastructure/modules/frontend/main.tf` — the stricter, authoritative one — names `us.i.posthog.com` and `eu.i.posthog.com` explicitly, alongside `*.sentry.io` for the shipped-but-unkeyed error rail. Nothing Google remains in either policy.
