...
| ------------------------- | ------------------ | ------------------------------------------------------------------------- | ----- | ---------- |
| Backend unit              | vitest             | `backend/tests/unit/{config,handlers,middleware,models,services,utils}`   | 90    | 1,285      |
...
**2,227 vitest cases** across 208 files — 1,491 backend, 736 frontend. The backend suite runs in ~17s and the frontend in ~80s (jsdom, serial by config).
