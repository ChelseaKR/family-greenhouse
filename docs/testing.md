...
| ------------------------- | ------------------ | ------------------------------------------------------------------------- | ----- | ---------- |
| Backend unit              | vitest             | `backend/tests/unit/{config,handlers,middleware,models,services,utils}`   | 91    | 1,321      |
...
| Frontend colocated unit   | vitest             | `frontend/src/**/*.test.ts`                                               | 12    | 49         |
...
**2,291 vitest cases** across 213 files — 1,527 backend, 764 frontend. The backend suite runs in ~17s and the frontend in ~80s (jsdom, serial by config).
